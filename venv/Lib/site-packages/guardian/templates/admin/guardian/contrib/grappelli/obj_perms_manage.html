...
<form action="." method="post">
    ...
    <div>
        {% if user_form.errors %}
        <p class="errornote">{% blocktrans count user_form.errors|length as counter %}Please correct the error below.{%
            plural %}Please correct the errors below.{% endblocktrans %}</p>
        <ul class="errorlist">{% for error in user_form.errors %}
            <li>{{ error }}</li>
            {% endfor %}
        </ul>
        {% endif %}
        <fieldset class="grp-module">
            ...
            <div class="grp-row grp-cells-1 user_permissions">
                <div class="l-2c-fluid l-d-4">
                    <div class="c-1"><label for="user-permissions" class="required">{% trans "User permissions"
                        %}</label></div>
                    <div class="c-2">
...
<form action="." method="post">
    ...
    <div>
        ...
        <p class="errornote">{% blocktrans count group_form.errors|length as counter %}Please correct the error below.{%
            plural %}Please correct the errors below.{% endblocktrans %}</p>
        <ul class="errorlist">{% for error in group_form.errors %}
            <li>{{ error }}</li>
            {% endfor %}
        </ul>
        {% endif %}
        <fieldset class="grp-module">
            ...
            <div class="grp-row grp-cells-1 group_permissions">
                <div class="l-2c-fluid l-d-4">
                    <div class="c-1"><label for="group-permissions" class="required">{% trans "Group permissions"
                        %}</label></div>
                    <div class="c-2">
